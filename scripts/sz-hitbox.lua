--[[
    SZ Hitbox Script v5.0 - WELD SYSTEM (REAL DAMAGE)
    - Creates hitbox parts WELDED into character (not separate in workspace)
    - Hitbox becomes part of character → game detects it for damage!
    - No physics desync - welds follow automatically
...
-- Create hitbox WELDED to character
local function createHitbox(player)
    ...
    local oldHitbox = character:FindFirstChild(HITBOX_NAME)
    ...
    hitbox.Anchored = false     -- Not anchored - weld will hold it
    hitbox.CastShadow = false
    hitbox.Massless = true      -- Don't affect character physics
    hitbox.Archivable = false
    
    -- Position at root part
    hitbox.CFrame = rootPart.CFrame
    
    -- IMPORTANT: Parent to CHARACTER (not workspace)
    hitbox.Parent = character
    
    -- Create WELD to attach hitbox to HumanoidRootPart
    local weld = Instance.new("WeldConstraint")
    weld.Name = HITBOX_NAME .. "Weld"
    weld.Part0 = rootPart
    weld.Part1 = hitbox
    weld.Parent = hitbox
...
function removeHitbox(player)
    ...
    if data then
        -- Destroy hitbox (it's in character, will clean up weld too)
...
local function updateHitboxSizes()
    for player, data in pairs(Hitboxes) do
        if data.Hitbox and data.Hitbox.Parent then
            data.Hitbox.Size = Vector3.new(Config.Size, Config.Size, Config.Size)
            data.Hitbox.Color = Config.Mode == "Enemy" and Config.Color or Config.TeamColor
...
InfoLabel.Text = "✅ Welded to character → Real damage!"
...
ToggleBtn.Position = UDim2.new(0, 0, 0, 24)
...
ModeBtn.Position = UDim2.new(0, 0, 0, 72)
...
SliderContainer.Position = UDim2.new(0, 0, 0, 124)
...
StatusLabel.Text = "Part in Character + Weld = ✅ Damage"
...
notify("🎯 SZ Hitbox v5", "Welded hitbox = Real damage!", 4)

if hasTeams() then
    StatusLabel.Text = "⚔️ Teams detected | Welded system"
else
...
print("[SZ Hitbox v5] Loaded!")
print("[SZ Hitbox v5] Hitbox is WELDED into character tree")
print("[SZ Hitbox v5] This means game detects it for DAMAGE!")
